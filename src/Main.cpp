...
void PyramidPlunder::door_open(void)
{
...
void PyramidPlunder::remove_components(DynamicArray<ecs::Entity>& entities)
{
...
void PyramidPlunder::start_game()
{
	scene.in_game = true;

	remove_component<Visibility>(background);
	remove_component<Visibility>(play_button);
	remove_component<Visibility>(exit_button);
...
void PyramidPlunder::start_level(const U8 id)
{
	...
	switch(id)
	{
		...
		case 2: level_manager.load_level("Levels/level3.txt"); break;
	}
