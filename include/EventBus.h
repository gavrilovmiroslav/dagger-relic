...
namespace core
{
	...
	struct MouseState
	{
		inline static const MouseState& get() 
		{
			static AccessUnique<MouseState> accessor;
			return accessor.access_unique();
		}
	...
	private:
		...
		containers::StaticArray<Bool, 3> current_mouse_buttons;
		containers::StaticArray<Bool, 3> previous_mouse_buttons;
	...
	struct KeyState
	{
		constexpr static const I8 Max_Keys_At_Same_Time = 20;
		using KeyStateBuffer = containers::SmallBuffer<KeyCode, I8, Max_Keys_At_Same_Time>;

		inline static const KeyState& get()
		{
			static AccessUnique<KeyState> accessor;
			return accessor.access_unique();
		}

		inline Bool is_down(KeyCode key) const
		{
			return current_keys.contains(key);
		}

		inline Bool is_up(KeyCode key) const
		{
			return !current_keys.contains(key);
		}

		inline Bool is_pressed(KeyCode key) const
		{
			return pressed_keys.contains(key);
		}

		inline Bool is_released(KeyCode key) const
		{
	...
	private:
		KeyStateBuffer current_keys;
		KeyStateBuffer pressed_keys;
		KeyStateBuffer released_keys;
	};
	...
	class EventBusModule
		...
		, public MutAccessUnique<KeyState>
