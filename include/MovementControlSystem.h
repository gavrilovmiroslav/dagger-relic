	...
	, public MutAccessGroupStorage<PlayerMovement, KeyBinding, Movement, SpriteAnimation>
{
	...
	for (auto&& [entity, player, key_binding, movement, sprite] : access_storage().each())
	{
		...
		else if ((player.p_fsm.currentState == PlayerState::RUNNING) && !key.is_down(key_binding.left) && !key.is_down(key_binding.right) && !key.is_down(key_binding.up) && !key.is_down(key_binding.down))
        	{
